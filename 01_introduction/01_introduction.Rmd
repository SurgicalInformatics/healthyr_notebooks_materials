...
* Run all lines in a code chunk with Ctrl+Shift+Enter


1. Practise these keyboard shortcuts on the code chunk below.
...
3. Click on `mydata` and investigate it in the new tab that opens up next to `01_introduction.Rmd`. What's the difference between `mydata` and `mydata2013`?
...
```{r}
# Run the source line (Control+Entrer) and check the Environment tab
...
After loading some data, you are ready to make your first R plot by runing the code below. We recommend Ctrl+Shift+Enter but you can also use the little green triangle at the top-right corner.
...
2. Change the word "fill" to "doge".
...
Going back to the tibble `mydata` with all years (copy the R code from Exercise 2), add `+ facet_wrap(~year)` to plot all years at once. (A *tibble* is what we call tables that have been loaded to R - so they show up in the Environment tab.)
