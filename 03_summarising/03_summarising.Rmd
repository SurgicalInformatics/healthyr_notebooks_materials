...
The arguments `names_to` and `values_to` are not strictly necessary, if you omit them the columns will get assigned defaults - "name" and "value". Delete the arguments `names_to = "sex", values_to = "percentage"` from the code below, so the function becomes `pivot_longer(-year)` to convince yourself the previous sentence holds true.
