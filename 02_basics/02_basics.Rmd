...
## Specifying variable types - worked example

Run each of these lines (Ctrl+Enter) to see what they return.
...
If we wanted to add in a new column or variable to our data, we can simply can use the dollar sign '$' to create a new variable inside a pre-existing piece of data. Run these lines and click on `mydata` in the Environment tab to check this worked as expected:
...
If you click on mydata in the Environment tab after running the code chunk with `select(-new)`, you will see that the variable is still included. That's because we didn't use the assignemt operator when performing the select. So it got printed for us to see, but not saved to the Environment.

1. Copy the example above and add in `mydata = ` on the first line.
2. Then drop both `new` as well as `new2` from mydata.
...
* If you misspell a variable name when using select positively (e.g. to only select one variable, such as `mydata %>% select(cuase)`), it will give a much better error of `Error in .f(.x[[i]], ...) : object 'cuase' not found`.
...
In this simple case, the solution using `mutate()` will actually be a bit longer than the original `$` way of doing it. However, in most cases, `mutate()` is much better, as you can potentially include multiple variables. So for the rest of this course, we will be adding and modiying columns using `mutate()`, and not `$`. But the `$` is useful for quickly printing out a column.
...
Round the new column `deaths_millions` to no decimals. Hint: copy a `mutate()` example from above modify it, rather than typing in `mydata = mydata %>% mutate()...` by hand.This is not only faster, but this methodology of copy-paste-edit makes even more sense once the examples get longer. Furthermore, the workflow of Google-copy-paste-edit is appropriate both when learning programming, but professionals also use it to look up things they don't use frequently enough to memorise them.
